...
export default function App() {
    ...
    const handleChallengeClose = () => {
        if (pendingInteraction) {
            ...
            const nextPlayerIndex = (pendingInteraction.playerIndex + 1) % players.length;
    ...
    const handleGreenConfirm = () => {
        setGreenModal(false);
        // Maju 1 langkah
        const currentPlayer = players[currentPlayerIndex];
        const newPos = Math.min(currentPlayer.position + 1, 100);
        setPlayers(prev => prev.map((p, i) => i === currentPlayerIndex ? { ...p, position: newPos } : p));
        // Setelah maju, akhiri giliran
...
const handleInteractionResult = useCallback((wasSuccessful: boolean, playerIndex: number) => {
    ...
    const handleQuestionClose = () => {
        if (pendingInteraction) {
            setQuestionState({ isVisible: false, question: null });
            setPendingInteraction(null);
            // Ganti giliran ke player berikutnya
            const nextPlayerIndex = (pendingInteraction.playerIndex + 1) % players.length;
            setCurrentPlayerIndex(nextPlayerIndex);
    ...
    const processTileRules = useCallback((position: number, playerIndex: number) => {
        ...
        if (tileType === TileType.GREEN) {
            ...
            // Tambahkan jeda 1 detik sebelum modal muncul
            setTimeout(() => {
                setGreenModal(true); // Tampilkan modal konfirmasi
            }, 1000);
            return;
        }
    ...
    if (ruleResult.interaction) {
            ...
            if (ruleResult.interaction.type === 'question') {
                // Tambahkan jeda animasi sebelum modal muncul
                setTimeout(() => {
                ...
                }, 1000); // Jeda 1 detik setelah animasi berhenti
    ...
    return (
        <div className="min-h-screen text-[#1E459F] flex flex-col items-center justify-center p-4">
                        ...
                        {gameStatus === 'IN_PROGRESS' && (
                            <>
                                ...
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-full transition-all duration-300 ${isDebugMode ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600'} border-2 border-white shadow-lg`}
           ...
           <QuestionModal
                ...
                onComplete={handleQuestionClose}
            />
            <ChallengeModal
            ...
            />
                    ...
                    {/* Modal untuk Kotak Hijau */}
                    {greenModal && (
                        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] backdrop-blur-sm">
                            <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border-4 border-green-400">
                                <div className="bg-green-100 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <span className="text-4xl">🎉</span>
                                </div>
                                
                                <h2 className="text-3xl font-black text-green-600 mb-2 font-display">Selamat!</h2>
                                <div className="bg-gray-50 p-6 rounded-xl mb-6 border-2 border-dashed border-gray-200">
                                    <p className="text-xl font-bold text-gray-800 leading-relaxed">
                                        Kamu mendapatkan 1 langkah tambahan!
                                    </p>
                                </div>

                                <button 
                                    onClick={handleGreenConfirm}
                                    className="bg-green-500 hover:bg-green-600 text-white py-4 rounded-xl font-black text-lg shadow-lg transform transition active:scale-95 w-full"
                                >
                                    OK
                                </button>
                            </div>
